...
const AdminPanelProductFormEdit = (props) => {
  ...
  const [opcijeNiz, setOpcijeNiz] = useState([...item.opcije]);
  const [slikaBlob, setSlikaBlob] = useState(item.slika);
  const [formState, setFormState] = useState({ ...item });

  useEffect(() => {
    setOpcijeNiz([...item.opcije]);
    setSlikaBlob(item.slika);
    setFormState({ ...item });
  }, [item]);
  ...
  const handleSubmit = (e) => {
    ...
    if (validator(formState)) {
      ...
      axios.patch(api, {
        ...
        .catch((err) => {
          console.log('cathct error in http://localhost:3001/api/product/update', err);
        })
  ...
  return (
    <div className="modal-form">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <br />
        <button className="btn-round" type="submit"><i className="fa fa-plus" aria-hidden="true"></i> Sačuvaj izmene</button>
...
export default AdminPanelProductFormEdit;
